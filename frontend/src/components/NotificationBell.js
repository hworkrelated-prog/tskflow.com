...
export const NotificationBell = () => {
    ...
    return (
        <div ref={wrapperRef} className="relative">
            ...
            {open && (
                <div
                    className="notif-panel fixed sm:absolute inset-x-3 sm:inset-x-auto sm:right-0 top-[calc(3.75rem+env(safe-area-inset-top,0px))] sm:top-12 w-auto sm:w-96 max-w-none sm:max-w-[92vw] bg-popover text-popover-foreground border border-border rounded-xl shadow-xl z-50 max-h-[min(70dvh,28rem)] flex flex-col overflow-hidden"
                    data-testid="notification-panel"
                >
                    <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                        <h3 className="font-semibold text-foreground">Notifications</h3>
                        {unread > 0 && (
                            <button onClick={markAll} className="text-xs text-teal-600 dark:text-teal-400 hover:underline flex items-center gap-1">
                    ...
                    <button
                        ...
                        className="notif-catch-up w-full px-4 py-2.5 text-left border-b border-border text-sm flex items-center gap-2"
                        data-testid="catch-up-link"
                    >
                        <Inbox className="w-4 h-4 shrink-0" />
                        <span className="font-medium">Catch up on what&apos;s due</span>
                        <span className="ml-auto text-xs opacity-80">Review →</span>
                    </button>
                    <button
                        type="button"
                        onClick={() => { setOpen(false); navigate('/updates'); }}
                        className="notif-whats-new w-full px-4 py-2.5 text-left border-b border-border text-sm flex items-center gap-2"
                        data-testid="whats-new-link"
                    >
                        <span className="text-base">✨</span>
                        <span className="font-medium">What&apos;s new in Tskflow</span>
                        <span className="ml-auto text-xs opacity-80">See changelog →</span>
                    </button>
                    <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain">
                        {items.length === 0 ? (
                            <div className="px-6 py-10 text-center text-sm text-muted-foreground">You&apos;re all caught up.</div>
                        ) : items.map((n) => (
                            <div
                                key={n.id}
                                role="button"
                                ...
                                className={`w-full text-left px-4 py-3 border-b border-border last:border-0 hover:bg-muted flex items-start gap-3 cursor-pointer ${n.read ? 'bg-popover' : 'notif-unread'}`}
