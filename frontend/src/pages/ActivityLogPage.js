        ...
        <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-teal-50/40">
            <header className="border-b bg-white/80 backdrop-blur sticky top-0 z-20">
                <div className="container mx-auto px-4 py-3 flex items-center gap-3 max-w-6xl">
                    <Button variant="ghost" size="icon" className="rounded-full" onClick={() => navigate('/dashboard')} data-testid="activity-back">
                        <ArrowLeft className="w-5 h-5" />
                    </Button>
                    <div className="flex-1 min-w-0">
                        <h1 className="text-xl font-bold truncate" style={{ fontFamily: 'Outfit' }}>Activity & data log</h1>
                        <p className="text-xs text-muted-foreground truncate">
                            Assigner, assignee, times, reminders, chatter — viewable and exportable
                            {user?.company_domain ? ` · ${user.company_domain}` : ''}
                        </p>
                    </div>
                    <Button onClick={downloadCsv} disabled={exporting} className="rounded-full" data-testid="activity-export-csv">
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                    <Card className="border rounded-2xl">
                        <CardHeader className="pb-3">
                            ...
                            <CardDescription>
                                Every tracked field for tasks you can see, including reminders sent and chatter.
                            </CardDescription>
